<template>
  ...
  >
    <div
      class="rounded-2xl overflow-hidden border border-white/8 shadow-2xl"
      style="background: rgba(18, 18, 22, 0.95); backdrop-filter: blur(24px)"
    >
      <!-- Search Input -->
      <div class="flex items-center gap-3 px-5 py-4 border-b border-white/6">
        ...
        <div v-if="searchQuery" class="flex items-center gap-1.5">
          <kbd
            class="px-1.5 py-0.5 text-[10px] font-mono rounded bg-white/8 text-white/40 border border-white/6"
            >↑↓</kbd
          >
          <kbd
            class="px-1.5 py-0.5 text-[10px] font-mono rounded bg-white/8 text-white/40 border border-white/6"
      ...
      <div class="max-h-[420px] overflow-y-auto spotlight-scrollbar">
        <div v-if="searchQuery.trim()" class="py-1">
          ...
          <template v-if="filteredInstances.length > 0">
            <div
              class="px-4 pt-3 pb-1.5 text-[10px] font-600 uppercase tracking-widest text-white/30 flex items-center gap-2"
            >
              <svg
                width="14"
                height="14"
            ...
            <div
              ...
              :class="
                highlightedIndex === getInstanceIndex(index)
                  ? 'bg-white/8'
                  : 'hover:bg-white/4'
              "
            ...
            >
              <div
                style="
                  ...
                  border-radius: 4px;
                "
                class="overflow-hidden flex-shrink-0 bg-white/5 flex items-center justify-center"
              >
                <v-img
                  v-if="instance.icon"
                  :src="instance.icon"
              ...
              <div class="flex-1 min-w-0">
                <div class="text-sm font-500 text-white/90 truncate">
                  {{ instance.name }}
                </div>
                <div class="flex items-center gap-1.5 mt-0.5">
                  <span
                    class="text-[11px] px-1.5 py-px rounded bg-white/6 text-white/40 font-mono"
                    >{{ instance.runtime?.minecraft }}</span
                  >
          ...
          <template v-if="modResults.length > 0">
            ...
            >
              ...
              >
                <path
                  d="M20.5 11H19V7c0-1.1-.9-2-2-2h-4V3.5a2.5 2.5 0 0 0-5 0V5H4c-1.1 0-2 .9-2 2v3.8h1.5a2.7 2.7 0 0 1 0 5.4H2V20c0 1.1.9 2 2 2h3.8v-1.5a2.7 2.7 0 0 1 5.4 0V22H17c1.1 0 2-.9 2-2v-4h1.5a2.5 2.5 0 0 0 0-5z"
                />
              </svg>
              {{ t("spotlight.sections.mods") }}
              <span
                v-if="currentGameVersion"
                class="text-white/20 font-mono normal-case"
                >· {{ currentGameVersion }}</span
              >
            </div>
            <div
              ...
              class="mx-2 px-3 py-2.5 rounded-xl cursor-pointer flex items-center gap-3 transition-all duration-150"
              :class="
                highlightedIndex === getModIndex(index)
                  ? 'bg-white/8'
                  : 'hover:bg-white/4'
              "
            ...
            >
              ...
              <div class="flex-shrink-0 flex items-center">
                <!-- Installing spinner -->
                <div
                  v-if="installingMods[mod.id]"
                  class="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-white/5"
                >
                  <v-progress-circular
                    indeterminate
                    size="14"
                  ...
                  <span class="text-[11px] text-blue-400">{{
                    t("spotlight.installing")
                  }}</span>
                </div>
                <!-- Installed check -->
                <div
                  v-else-if="installedMods[mod.id]"
                  class="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-emerald-500/10"
                >
                  <svg
                    width="14"
                    height="14"
                  ...
                  <span class="text-[11px] text-emerald-400">{{
                    t("spotlight.installed")
                  }}</span>
                </div>
                <!-- Install failed -->
                <div
                  v-else-if="failedMods[mod.id]"
                  class="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-red-500/10"
                >
                  <svg
                    width="14"
                    height="14"
                  ...
                  <span class="text-[11px] text-red-400">{{
                    t("spotlight.installFailed")
                  }}</span>
                </div>
                <!-- Install button -->
                <div
                  v-else
                  class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-500/12 text-emerald-400 hover:bg-emerald-500/20 transition-colors"
                >
                  <svg
                    width="14"
                    height="14"
          ...
          <template v-if="localModResults.length > 0">
            ...
            <div
              ...
              class="mx-2 px-3 py-2.5 rounded-xl cursor-pointer flex items-center gap-3 transition-all duration-150"
              :class="
                highlightedIndex === getLocalModIndex(index)
                  ? 'bg-white/8'
                  : 'hover:bg-white/4'
              "
            ...
            >
              ...
              <div
                v-if="mod.installed && mod.installed.length > 0"
                class="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/10"
              >
                <svg
                  width="12"
                  height="12"
          ...
          <div
            ...
            "
            class="px-6 py-10 flex flex-col items-center gap-2 text-center"
          >
            <svg
              width="32"
              height="32"
        ...
        <div v-else class="py-1">
          ...
          <div
            ...
            class="mx-2 px-3 py-2.5 rounded-xl cursor-pointer flex items-center gap-3 transition-all duration-150"
            :class="
              highlightedIndex === index ? 'bg-white/8' : 'hover:bg-white/4'
            "
            @click="action.action"
          ...
          >
            ...
            >
              <svg
                ...
                stroke-linecap="round"
                stroke-linejoin="round"
                v-html="action.svgPath"
              />
            </div>
            <div class="text-sm font-500 text-white/80">
          ...
          <template v-if="recentInstances.length > 0">
            ...
            <div
              ...
              class="mx-2 px-3 py-2.5 rounded-xl cursor-pointer flex items-center gap-3 transition-all duration-150"
              :class="
                highlightedIndex === quickActions.length + idx
                  ? 'bg-white/8'
                  : 'hover:bg-white/4'
              "
              @click="selectInstance(instance)"
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <div class="text-sm font-500 text-white/80 truncate">
                  {{ instance.name }}
                </div>
      ...
      <!-- Footer -->
      <div
        class="px-4 py-2 border-t border-white/5 flex items-center justify-between"
      >
        ...
        <div
          v-if="currentGameVersion && currentModLoader"
          class="flex items-center gap-1.5 text-[10px] text-white/20"
        >
...
const currentGameVersion = computed(
  () => currentInstance.value?.runtime?.minecraft
);
...
const quickActions = computed(() => [
  ...
  },
  {
    ...
    label: "mods",
    svgPath:
      '<path d="M20.5 11H19V7c0-1.1-.9-2-2-2h-4V3.5a2.5 2.5 0 0 0-5 0V5H4c-1.1 0-2 .9-2 2v3.8h1.5a2.7 2.7 0 0 1 0 5.4H2V20c0 1.1.9 2 2 2h3.8v-1.5a2.7 2.7 0 0 1 5.4 0V22H17c1.1 0 2-.9 2-2v-4h1.5a2.5 2.5 0 0 0 0-5z" fill="rgba(96,165,250,0.7)" stroke="none"/>',
    iconColor: "rgba(96,165,250,0.7)",
    bgClass: "bg-blue-500/10",
    action: () => navigateToMods(),
  },
  {
    id: "store",
    label: "store",
    svgPath:
      '<path d="M3 9l1-4h16l1 4M3 9v11a1 1 0 0 0 1 1h16a1 1 0 0 0 1-1V9M3 9h18M9 21V9M15 21V9M12 5V3"/>',
    iconColor: "rgba(52,211,153,0.7)",
    bgClass: "bg-emerald-500/10",
    action: () => navigateToStore(),
  },
]);
...
const { modResults, localModResults, isSearching } = useSpotlightSearch(
  searchQuery,
  currentGameVersion,
  currentModLoader
);
...
const getLocalModIndex = (index: number) =>
  filteredInstances.value.length + modResults.value.length + index;
...
function selectHighlighted() {
  ...
  if (highlightedIndex.value < filteredInstances.value.length) {
    ...
    filteredInstances.value.length + modResults.value.length
  ) {
  ...
  try {
    ...
    for (const dep of requiredDeps) {
      if (dep.version_id) {
        ...
        try {
          const depVersions = await clientModrinthV2.getProjectVersions(
            dep.project_id,
            {
              loaders: [loader],
              gameVersions: [gameVersion],
    ...
    if (e.name === "InstanceUpstreamError") {
      // Instance is busy, don't mark as failed and don't log as error
      delete installingMods[mod.id];
      return;
    }
